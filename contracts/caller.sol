        ...
        unchecked {
            ...
            for (uint256 i; i <= msg.data.length - 0x20; i += 0x20) {
                ...
                if (poolCall & PID_MASK == UNIV2_PID) {
                    ...
                    assembly {
                        mstore(0, tokenSel)
                        pop(staticcall(gas(), pool, 0, 0x04, 0, 0x20))
                        let token := mload(0)
                        mstore(0, TRANSFER_SEL)
                        mstore(0x04, pool)
                        mstore(0x24, amIn)
                        pop(call(gas(), token, 0, 0, 0x44, 0, 0))
                    }
                    ...
                    assembly {
                        mstore(0x04, amOut0)
                        mstore(0x24, amOut1)
                        if iszero(call(gas(), pool, 0, 0, 0xa4, 0, 0)) {
                            revert(0, 0)
                        }
                    }
                } else {
                    ...
                    require((amIn = uint256(-amDel)) >= uint256(uint48(poolCall >> 168)) << uint8(poolCall >> 160));
                }
            }
        }
    }
